"""
Async HTTP client for API mode operations.

Provides async HTTP communication with retry logic and error handling.
"""
...
class AsyncCortexHTTPClient:
    """
    Async HTTP client for Cortex API mode operations.

    Handles async HTTP requests with automatic retry, error mapping, and authentication.
    ...
    Examples:
        >>> client = AsyncCortexHTTPClient(host="http://localhost:9002/api/v1")
        >>> response = await client.get("/metrics", params={"environment_id": env_id})
    """
    ...
    async def post(
        ...
        Examples:
            >>> response = await client.post("/metrics", data={"name": "Revenue"})
        """
    ...
    async def put(
        ...
        Examples:
            >>> response = await client.put("/metrics/123", data={"name": "Updated"})
        """
    ...
    async def patch(
        ...
        Examples:
            >>> response = await client.patch("/metrics/123", data={"name": "Patched"})
        """
    ...
    async def delete(
        ...
        Examples:
            >>> response = await client.delete("/metrics/123")
        """
    ...
    async def upload_file(
        ...
        """
        Upload file with multipart/form-data encoding (async).
        ...
        Examples:
            >>> response = await client.upload_file("/files", "data.csv")
            >>> response = await client.upload_file("/files", file_bytes, filename="data.csv")
        """
        ...
        response = await self._request("POST", endpoint, params=params, files=files)
        return response.json()
    ...
    async def download_file(
        ...
        Examples:
            >>> content = await client.download_file("/files/123/download")
