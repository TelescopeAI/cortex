          ...
          <div class="space-y-3">
            ...
            <div class="flex items-center gap-2 flex-wrap">
              ...
              <template v-if="filter.operator && !['is_null', 'is_not_null'].includes(filter.operator)">
                <!-- Single Value -->
                <template v-if="!['in', 'not_in', 'between', 'not_between'].includes(filter.operator)">
                  <!-- Date/Timestamp Picker -->
                  <Popover v-if="['date', 'timestamp'].includes(filter.value_type)">
                    <PopoverTrigger as-child>
                      <Button
                        variant="outline"
                        :class="cn('w-auto min-w-[200px] justify-start text-left font-normal h-9', !filter.value && 'text-muted-foreground')"
                      >
                        <CalendarIcon class="mr-2 h-4 w-4" />
                        {{ filter.value ? filter.value : "Pick a date" }}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent class="w-auto p-0" align="start">
                      <Calendar
                        :model-value="filter.value ? parseDate(filter.value.split('T')[0]) : undefined"
                        @update:model-value="(date: DateValue | undefined) => date && updateFilter(index, 'value', date.toString())"
                  ...
                  <!-- Boolean Toggle -->
                  <div v-else-if="filter.value_type === 'boolean'" class="flex items-center gap-2">
                    <Switch
                      :model-value="filter.value === 'true' || filter.value === true"
                      @update:model-value="(value) => updateFilter(index, 'value', value ? 'true' : 'false')"
                    />
                    <span class="text-sm">{{ filter.value === 'true' || filter.value === true ? 'True' : 'False' }}</span>
                  </div>
                ...
                <template v-else-if="['between', 'not_between'].includes(filter.operator)">
                  ...
                  <template v-if="['date', 'timestamp'].includes(filter.value_type)">
                    <Popover>
                      <PopoverTrigger as-child>
                        <Button
                          variant="outline"
                          :class="cn('w-[140px] justify-start text-left font-normal h-9', !filter.min_value && 'text-muted-foreground')"
                        >
                          <CalendarIcon class="mr-2 h-4 w-4" />
                          {{ filter.min_value ? filter.min_value : "From" }}
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent class="w-auto p-0" align="start">
                        <Calendar
                          :model-value="filter.min_value ? parseDate(filter.min_value.split('T')[0]) : undefined"
                          @update:model-value="(date: DateValue | undefined) => date && updateFilter(index, 'min_value', date.toString())"
                          :default-placeholder="today(getLocalTimeZone())"
                          layout="month-and-year"
                          initial-focus
                        />
                    ...
                    <Popover>
                      <PopoverTrigger as-child>
                        <Button
                          variant="outline"
                          :class="cn('w-[140px] justify-start text-left font-normal h-9', !filter.max_value && 'text-muted-foreground')"
                        >
                          <CalendarIcon class="mr-2 h-4 w-4" />
                          {{ filter.max_value ? filter.max_value : "To" }}
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent class="w-auto p-0" align="start">
                        <Calendar
                          :model-value="filter.max_value ? parseDate(filter.max_value.split('T')[0]) : undefined"
                          @update:model-value="(date: DateValue | undefined) => date && updateFilter(index, 'max_value', date.toString())"
                          :default-placeholder="today(getLocalTimeZone())"
                          layout="month-and-year"
                          initial-focus
                        />
                  ...
                  <!-- Number Range -->
                  <template v-else-if="filter.value_type === 'number'">
                    <Input
                      type="number"
...
import { Plus, Filter, X, Settings, ChevronDown, CalendarIcon } from 'lucide-vue-next'
