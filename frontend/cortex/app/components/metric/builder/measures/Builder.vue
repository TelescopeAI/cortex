...
const handleNameChange = (measure: Measure, newName: string | number) => {
  ...
  // Only auto-fill alias if it was previously auto-filled
  if (!measure.alias || measure.alias === toSnakeCase(measure.name)) {
    measure.alias = snakeCaseName;
  }
  
  // NEVER update query - it's the actual database column selected by the user
  // Query should remain independent of the display name
